...
INPUT_SCHEMA_MAPPER = {
    ...
    'transform_todo_summary': TODO_SUMMARY_INPUT_SCHEMA
}
...
def validate_df_input(df: pd.DataFrame, func_name: str) -> pd.DataFrame:
    ...
    schema = INPUT_SCHEMA_MAPPER.get(func_name)
...
def validate_df_output(df: pd.DataFrame, func_name: str) -> pd.DataFrame:
    ...
    schema = OUTPUT_SCHEMA_MAPPER.get(func_name)
